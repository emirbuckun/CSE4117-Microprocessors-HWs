// to compile, gcc main.c -o assembler
...
// how to compile: gcc assembler.c -o assembler
// how to run: ./assembler < code.asm > RAM
#include <stdio.h>
...
int main() {
    ...
    if (fp != NULL) {
            ...
            while (token) {
                if (strcmp(token, "ldi") == 0) //---------------LDI INSTRUCTION--------------------
                {
                    ...
                    program[counter] = 0x1000 +
                                       hex2int(op1); //generate the first 16-bit of the ldi instruction
                    counter++; //move to the second 16-bit of the ldi instruction
                    if ((op2[0] == '0') &&
                        (op2[1] == 'x')) //if the 2nd operand is twos complement hexadecimal
                        program[counter] = hex2int(op2 + 2) &
                                           0xffff; //convert it to integer and form the second 16-bit
                    else if (((op2[0]) == '-') ||
                             ((op2[0] >= '0') && (op2[0] <= '9'))) //if the 2nd operand is decimal
                        program[counter] = atoi(op2) &
                                           0xffff; //convert it to integer and form the second 16-bit
                    ...
                    { //in this case, the 2nd 16-bits of the ldi instruction cannot be generated.
                        ...
                        op1 = (char *) malloc(
                                sizeof(op2)); //and the name of the label/variable that it must contain
                ...
                } else if (strcmp(token, "ld") == 0) //------------LD INSTRUCTION---------------------
                {
                    ...
                    ch = (op1[0] - 48) |
                         ((op2[0] - 48) << 3); //form bits 11-0 of machine code. 48 is ASCII value of '0'
                ...
                } else if (strcmp(token, "st") == 0) //-------------ST INSTRUCTION--------------------
                {
                    ...
                    op2 = strtok(NULL,
                                 "\n\t\r "); //get the 2nd operand of ld, which is the source register
                    chch = (op1[0] - 48) << 3 |
                           ((op2[0] - 48) << 6); //form bits 11-0 of machine code. 48 is ASCII value of '0'
                ...
                } else if (strcmp(token, "add") == 0) //----------------- ADD -------------------------------
                {
                    ...
                    // TODO 0111 + 3 bit ALU CODE (0001) + 3 bit R2 + 3 bit R3 + 3 bit R1
                    // 0111 0010 0000 0000
                    op1 = strtok(NULL, "\n\t\r ");
                    op2 = strtok(NULL, "\n\t\r ");
                    op3 = strtok(NULL, "\n\t\r ");
                    ...
                    // TODO 0111 + 3 bit ALU CODE (0010) + 3 bit R2 + 3 bit R3 + 3 bit R1
                    // 0111 0100 0000 0000
                    op1 = strtok(NULL, "\n\t\r ");
                    op2 = strtok(NULL, "\n\t\r ");
                    op3 = strtok(NULL, "\n\t\r ");
                    ...
                    // TODO 0111 + 3 bit ALU CODE (0011) + 3 bit R2 + 3 bit R3 + 3 bit R1
                    // 0111 0110 0000 0000
                    op1 = strtok(NULL, "\n\t\r ");
                    op2 = strtok(NULL, "\n\t\r ");
                    op3 = strtok(NULL, "\n\t\r ");
                    ...
                    // TODO 0111 + 3 bit ALU CODE (0100) + 3 bit R2 + 3 bit R3 + 3 bit R1
                    // 0111 1000 0000 0000
                    op1 = strtok(NULL, "\n\t\r ");
                    op2 = strtok(NULL, "\n\t\r ");
                    op3 = strtok(NULL, "\n\t\r ");
                    ...
                    // TODO 0111 + 6 bit ALU CODE (0110) + 3 bit R2 + 3 bit R1
                    // 0111 000110 000 000
                    // 0111 110110 000 000
                    // 0111 111001 000 000
                    op1 = strtok(NULL, "\n\t\r ");
                    op2 = strtok(NULL, "\n\t\r ");
                    ch = (op1[0] - 48) | ((op2[0] - 48) << 3);
                    program[counter] = 0x7E40 + ((ch) & 0x01ff);
                    counter++;
                } else if (strcmp(token, "inc") == 0) {
                    // TODO 0111 + 6 bit ALU CODE (0111) + 3 bit R1 + 3 bit R1
                    // 0111 000111 000 000
                    // 0111 110111 000 000
                    // 0111 111010 000 000
                    op1 = strtok(NULL, "\n\t\r ");
                    ...
                    // TODO 0111 + 6 bit ALU CODE (1000) + 3 bit R1 + 3 bit R1
                    // 0111 001000 000 000
                    // 0111 111000 000 000
                    // 0111 111011 000 000
                    op1 = strtok(NULL, "\n\t\r ");
                    ...
                    // TODO 1000 + 3 bit dummy + 3 bit R + 6 bit dummy
                    // push r
                    op1 = strtok(NULL, "\n\t\r ");
                    op2 = strtok(NULL, "\n\t\r ");
                    op3 = strtok(NULL, "\n\t\r ");
                    chch = (op1[0] - 48) | ((op2[0] - 48) << 6) | ((op3[0] - 48) << 3);
                    program[counter] = 0x8000 + ((chch) & 0x00f0);
                    counter++;
                } else if (strcmp(token, "pop") == 0) {
                    // TODO 1001 + 9 bit dummy + 3 bit R
                    // pop r
					op1 = strtok(NULL, "\n\t\r ");
                    ch = (op1[0] - 48);
                ...
                {
                    // TODO 1010 + 12 bit x (jumps to PC+x)
                    // call x
                    op1 = strtok(NULL, "\n\t\r ");
                    ch = (op1[0] - 48);
                    program[counter] = 0xA000 + ((ch) & 0x0fff);
                    counter++;
                } else if (strcmp(token, "ret") == 0) {
                    // TODO 1011 + 12 bit dummy
                    // ret x
                    program[counter] = 0xB000;
                    counter++;
                } else if (strcmp(token, "iret") == 0) {
                    // TODO 1110 + 12 bit dummy
                    // iret
                    program[counter] = 0xE000;
                    counter++;
                } else if (strcmp(token, "sti") == 0) {
                    // TODO 1100 + 12 bit dummy
                    // sti
                    program[counter] = 0xC000;
                    counter++;
                } else if (strcmp(token, "cli") == 0) {
                    // TODO 1101 + 12 bit dummy
                    // cli
        ...
        {
            j = 0;
            while ((j < nooflabels) && (strcmp(jumptable[i].name, labeltable[j].name) !=
                                        0)) //if the label for this jump/jz does not match with the
                j++; // jth label in the labeltable, check the next label..
            program[jumptable[i].location] += (labeltable[j].location - jumptable[i].location - 1) &
                                              0x0fff; //copy the jump address into memory.
        }
