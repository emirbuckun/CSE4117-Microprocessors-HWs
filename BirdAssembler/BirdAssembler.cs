    ...
    public static void Main()
    {
        ...
        try
        {
            ...
            while ((line = reader.ReadLine()!) != null)
            {
                ...
                void HandleJZ(string[] tokens)
                { // JZ 0x10
                    if (tokens.Length < 2) { Console.WriteLine("JZ requires 1 operand."); return; }
                    op1 = tokens[1];
                    int instructionReg = (op1.StartsWith("0x") ? Hex2Int(op1[2..]) : int.Parse(op1)) & 0x0FFF;
                    program[counter++] = (OPCODE_BASE_JZ + instructionReg).ToString("X4");
                }

                void HandleJMP(string[] tokens)
                { // JMP 0x10
                ...
                void HandleRET(string[] tokens)
                { // RET
                    program[counter++] = OPCODE_BASE_RET.ToString("X4");
                }

                void HandleALUInstruction(string[] tokens, ALUOp1 aluOp1, ALUOp2? aluOp2 = null)
                { // ADD r1 r2 r3, NOT r1 r2, INC r1
